test_that("calcMetaPvaluesVec works", {
    ...
    pvals <- c(bad_pvals, one_pvals, onesmall_pvals, p5_pvals)
    ...
    expect_equal(mpvals$NElements,c(4, 4, 3, 3))
    expect_equal(mpvals$Meta.pvalue, c(1.0, 1.0, 0.5, 0.5))

    mpvals <- calcMetaPValuesVec(pvals, by_list = by_list, "min")
    expect_equal(mpvals$NElements,c(4, 4, 3, 3))
    expect_equal(mpvals$Meta.pvalue, c(1.0, 1.0, 1e-17, 0.5))

    mpvals <- calcMetaPValuesVec(pvals, by_list = by_list, "wmin1")
    expect_equal(mpvals$NElements,c(4, 4, 3, 3))
    expect_equal(mpvals$Meta.pvalue, c(1.0, 1.0, 2.9e-17, 0.875))
