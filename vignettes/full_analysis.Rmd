...
With the epitope segments and the probe p-values, a
...
segments that involve clustering across samples.
...
## Making z-score level calls

Some investigators would like to make z-score level calls global rather then
using adjusted p-values.  HERON is flexible to allow for such setup.
For example, the user wanted to make probe-level calls using a z-score cutoff > 3.
