...
First, we start with a sequence *data.frame* where rows are amino acid sequences 
of the peptide binding probe and the columns are samples.   
...
prot_padj_uniq <- calcProteinPValuesMat(epi_padj_uniq, metap_method = "tippets");
```
...
then use the *calcEpitopePValuesMat* and *makeEpitopeCalls* functions as before to
...
*min_bonf*, *minFDR*, *maxFDR*, *min*, *fischer*/*sumlog*, *stouffer*/*sumz*, 
*meanz*, *lancaster*, *invt*, *logitp*, *meanp*, *edgington/sump*, 
*hmp/harmonicmeanp*, *wilkinsons_min1/tippets*, *wilkinsons_min2/wmin2*, 
*wilkinsons_min3*, *wilkinsons_min4*, *wilkinsons_min5*, *wilkinsons_max1/wmax1*
, *wilkinsons_max2/wmax2*, and *cct*.
...
the protein should be one that requires at least of the epitope p-values to be 
small (e.g. *wmax1*).  Other p-value methods such as the *cct* and the *hmp*
